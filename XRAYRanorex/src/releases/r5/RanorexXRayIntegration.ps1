...
class XrayTestEntityVo
{
	...
	[string]escapeSpecialCharacter($str){
		return $str.Replace("#", "\u0023").Replace(" ","\u0020").Replace("+","\u002B").Replace(".","\u002E").Replace(",","\u002C").Replace(";","\u003B").Replace("?","\u003F").Replace("|","\u01C0").Replace("*","\u002A").Replace("/","\u002F").Replace("%","\u0025").Replace("^","\u005E").Replace("$","\u0024").Replace("@","\u0040").Replace("[","\u005B").Replace("]","\u005D")
	}
	
	[boolean]exist(){
		$jql = $("project = " + [Constants]::projectKey + " AND issuetype = 'Test' AND `"Script Name`" ~ `"" + $this.escapeSpecialCharacter($this.description) + "`"");
...
class RanorexXmlProcessor{
    ...
    CreateTestPlanVo(){
        if([Constants]::isTestPlanGiven()){
            ...
            $this.testPlanExist = $true
        } else {
            $this.testPlanVo = [XrayTestPlanEntityVo]::new($this.testVos)
            $this.testPlanExist = $false
        }
    }

    SaveTestPlanVo(){
        if($this.testPlanExist){
            $this.testPlanVo.update()
        } else {
